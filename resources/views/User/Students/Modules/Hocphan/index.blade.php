...
<table id="classTable">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>{{ $Classroom->Day }}</td>
            <td>
                <button class="btn btn-xs btn-primary" style="margin-left:10px;padding:0;font-weight:700;background-color: #57eda8;border-color: #57eda8;">
                    <a href="{{ route('User.Detailclass', ['studentId' => $studentId, 'classId' => $Classroom->id]) }}" style="color: #fff;padding: 20px 20px;font-size: 0.75rem;line-height: 36px;">
                        Chi Tiết
                    </a>
                </button>
            </td>
            <td>
                <form action="{{ route('User.DeleteHocPhan', ['studentId' => $studentId,'classId' => $Classroom->id]) }}" method="POST" onsubmit="return confirm('Bạn có chắc chắn muốn xóa lớp này?')">
                    @csrf
                    @method('DELETE')
...
<div id="noResults" style="display: none; margin-top: 10px; color: red;">Không tìm thấy kết quả</div>
