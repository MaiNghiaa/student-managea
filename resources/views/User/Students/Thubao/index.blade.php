...
.another_info {
  ...
  <!-- / Menu -->
    ...
    <h1 class="line">Thông Báo</h1>
    <div class="main_content">
      {{-- {{dd($Get_detail, $student)}} --}}
      <ul>
        @if(count($malls_student) === 0)
            <li>Bạn không có thông báo nào</li>
        @endif
        @foreach ($malls_student as $mallstudent)
            @if ($mallstudent->type == 'success')
                <li class="alert alert-success">
                    <p class="text_content">{{$mallstudent->content}}</p>
                    <img src="{{asset('alert_img/success.png')}}" alt="" style="width:25px;height:25px; object-fit:cover;">
                </li>
            @elseif ($mallstudent->type == 'warning')
                <li class="alert alert-warning">
                    <p class="text_content">{{$mallstudent->content}}</p>
                    <img src="{{asset('alert_img/warning-icon-svg-10.jpg')}}" alt="" style="width:25px;height:25px; object-fit:cover;">
                </li>
                @elseif ($mallstudent->type == 'error')
                <li class="alert alert-error">
                    <p class="text_content">{{$mallstudent->content}}</p>
                    <img src="{{asset('alert_img/error-icon-svg-10.jpg')}}" alt="" style="width:25px;height:25px; object-fit:cover;">
                </li>
            @endif
        @endforeach
    </ul>
