...
.another_info {
  ...
  <div class="column_2">
	...
	<div class="limiter">	
		@if (session('warning'))
		<div id="error-warning" class="alert alert-warning">
			{{ session('warning') }}
		</div>
	@endif
    ...
    <div class="main_content">
      ...
      <h2 class="line">Thông tin Nộp Tiền </h2>
      {{-- {{dd($checks)}} --}}
      <div class="info">
        @foreach ($hocphi as $money)
          
        @endforeach
        @if ($thanhtoan->hocphi_id == $money->id)
        <p class="title">Loại Học Phí : 
          {{$money->name}}
        </p>
        @endif
        <p>Tiền học phí: {{ number_format($thanhtoan->TienHocPhi, 0, ',', '.') }} đ</p>
      </div>
      @if ($thanhtoan->status == 0)
      
      <form action="{{ route('user.FormHocPhi', ['studentId' => $studentId]) }}" method="POST" enctype="multipart/form-data">
        @csrf
        <label for="image">Ảnh minh chứng đã Nộp tiền </label>
        <input type="file" name="image">
        <button type="submit">Gửi</button>
    </form>
      @endif
      @if ($thanhtoan->status == 1)
        <div>Bạn Đã up ảnh lên vui lòng chờ trong một chút để tôi xác nhận</div>
      @endif
      @if ($thanhtoan->status == 2)
        <div>Bạn Đã Nộp tiền thành công</div>
      @endif
