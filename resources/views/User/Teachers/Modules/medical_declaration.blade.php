...
<div class="column_2">
  <aside id="layout-menu" class="layout-menu menu-vertical menu bg-menu-theme">
    ...
    <!-- Account Teacher -->
    <li class="menu-item">
      {{-- {{route('TeacherUser.ThuBao', ['teacherId' => $teacherId])}} --}}
  ...
  <!-- / Menu -->
    ...
    <div class="main_content">
      ...
      <h2 class="line">Thông tin y tế </h2>
      @if ($KBYT->status == 1)
      <label for="image">Đang kiểm tra, vui lòng đợi trong vài phút</label>
  @endif
  
  @if ($KBYT->status == 2)
      <label for="image">Đã Khai Báo Thành công</label>
      <img src="{{$KBYT->img}}" alt="">
  @endif
  
  @if ($KBYT->status == 0)
      <form action="{{ route('user.Teacher.post_khaibaoyte', ['teacherId' => $teacherId]) }}" method="POST" enctype="multipart/form-data">
          @csrf
          <label for="image">Ảnh minh chứng đã khai báo y tế</label>
