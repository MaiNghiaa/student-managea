...
<div class="column_2">
 ...
 <!-- / Menu -->
    ...
    <h1 class="line">Thông tin Lớp</h1>
    @if ($disableInputPoint)
    <button class="btn-input btn" style="color:#fff">Không được nhập điểm nữa đâu lewlew</button>
    @else
    <button class="btn-input btn"><a href="{{route('Teacher.InputPointStudent', ['teacherId' => $teacherId, 'classId' => $classId])}}" style="color:#fff;">Nhập Điểm Sinh Viên</a></button>
@endif
      ...
      <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    @php
                        $diemquatrinh = $bangdiem->where('student_id', $student->id)->pluck('diemquatrinh')->first();
                    @endphp
                ...
                <td>
                    @php
                        $gk = $bangdiem->where('student_id', $student->id)->pluck('Gk')->first();
                    @endphp
                    @if ($gk === null)
                        Đang cập nhật
                    @else
                ...
                <td>
                    @php
                        $ck = $bangdiem->where('student_id', $student->id)->pluck('ck')->first();
                    @endphp
                    @if ($ck === null)
                        Đang cập nhật
                    @else
                ...
                <td>
                  <button class="btn-input btn">
                    <a href="{{route('Teacher.changePointStudent', ['teacherId' => $teacherId, 'classId' => $classId,'student_id' => $student->id])}}" style="color:#fff;">sửa Điểm Sinh Viên</a>
                  </button>
                </td>
            </tr>
        @endforeach
