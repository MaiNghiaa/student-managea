...
@endif
  <div class="container">
    <div class="top">
        ...
        <div class="id_class df">
            <p class="heading">Mã Lớp</p>
            <p class="desc">{{$Classroom->classroom_id}}</p>
        </div>
        <div class="classname df">
            <p class="heading">Tên Môn Học</p>
            <p class="subject">{{$Classroom->classroom}}</p>
            @foreach ($Departments as $Department)
                @if ($Classroom->department_id == $Department->id)
                <p class="desc">{{$Department->Department}}</p>
                @endif
            @endforeach
        </div>
        <div class="teacher df">
            <p class="heading">Giảng Viên</p>
            @foreach ($teachers as $teacher)
            @foreach ($teachers_classrooms as $teacher_classroom)         
              @if ($teacher_classroom->classroom_id == $Classroom->id && $teacher_classroom->teacher_id == $teacher->id)

              <p class="desc">{{$teacher->Full_name}}</p>
              @endif
              @endforeach
              @endforeach
    ...
    <table>
      ...
      <tbody>
        ...
        @foreach ($students_classrooms as $student_classroom)
        @if ($student_classroom->classroom_id == $Classroom->id && $student_classroom->student_id == $student->id)
        ...
        {{-- {{dd($Classroom->id)}} --}}
          <tr>
            ...
            <td>
              {{-- <form class="form_del" action="{{ route('deleteClassroom', $Classroom->id) }}" method="POST">
                @csrf
                @method('DELETE')

                
                <button class="btn btn-xs btn-primary" style="margin-left:10px;padding:0;font-weight:700;
                background-color: #57eda8;
    border-color: #57eda8;">
    <a href="{{route('DetailClassroom',$Classroom->id)}}" style="color: #fff;
                  padding: 20px 20px;
                  font-size: 0.75rem;
                  line-height: 36px;
              }">
                  Chi Tiết </a></button>
                <button class="btn btn-xs btn-primary" style="margin:0px 15px;padding:0;"><a href="{{route('editClassroom',$Classroom->id)}}" style="color: #fff;
                  padding: 20px 20px;
                  font-size: 0.75rem;
                  line-height: 36px;
              }">
