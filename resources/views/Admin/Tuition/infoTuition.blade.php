...
@endif
  <div class="container">
      ...
      <tbody>
        ...
        {{-- {{dd($teacher->classroom_teacher())}} --}}
          <tr>
            ...
            @if ($tuition->status == 0)
            <td> Hết Hạn </td>
            @elseif ($tuition->status == 1)
              <td> Hoạt Động </td>
            @endif
            <td>
              <form class="form_del" action="{{ route('deleteTuition', $tuition->id) }}" method="POST">
                @csrf
                @method('DELETE')
               ...
               <button class="btn btn-xs btn-primary" style="margin-left:10px;padding:0;"><a href="{{route('editTuition',$tuition->id)}}" style="color: #fff;
                  padding: 20px 20px;
                  font-size: 0.75rem;
                  line-height: 36px;
              }">
                  ...
                  <form action="{{route('AddTuitiontoStudent', $tuition->id)}}" method="post">
                    <input type="text" name="TuitionId" id="TuitionId" hidden value="{{$tuition->id}}">
                  @csrf
                  <button type="submit" class="btn btn-xs btn-primary">Thêm cho học sinh</button>
                  </form>
          </td>
          </tr>
        @endforeach
    ...
    <div class="pagination">
      {{ $tuitions->links('pagination::bootstrap-4') }}
  </div>
  </div>
