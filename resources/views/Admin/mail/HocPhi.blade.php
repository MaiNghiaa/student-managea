...
@endif
  <div class="container">
    <table>
      ...
      <tbody>
        @foreach ($Mail_HocPhi as $Mail_Student)
        {{-- {{dd($teacher->classroom_teacher())}} --}}
          <tr>
            <td>{{ $loop->iteration }}</td>
            @foreach ($students as $student)
            @if ($Mail_Student->student_id == $student->id)
            <td>{{ $student->Full_name}}</td>
            @endif 
            @endforeach
                <td>
                    <img src="{{ $Mail_Student->image }}" alt="" style="min-width:100px;min-height:100px;max-width:300px;max-height:300px;object-fit:cover" class="fullscreen-trigger">
                </td>
                        <td>
                <form action="{{ route('update.HocPhiStatus', ['studentId' => $Mail_Student->student_id]) }}" method="POST">
                    @csrf
    ...
    <div class="pagination">
      {{ $Mail_HocPhi->links('pagination::bootstrap-4') }}
  </div>
  </div>
  <script>
