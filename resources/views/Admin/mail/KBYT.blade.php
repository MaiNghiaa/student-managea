...
@endif
  <div class="container">
    <table>
      ...
      <tbody>
        @foreach ($Mail_BHYT as $StudentMail)
        {{-- {{dd($teacher->classroom_teacher())}} --}}
          <tr>
            <td>{{ $loop->iteration }}</td>
            @foreach ($students as $student)
            @if ($StudentMail->id_student == $student->id)
            <td>{{ $student->Full_name}}</td>
            @endif    
            @endforeach
            <td>
                <img src="{{ $StudentMail->img }}" alt="" style="min-width:100px;min-height:100px;max-width:300px;max-height:300px;object-fit:cover" class="fullscreen-trigger">
            </td>
                        <td>
                <form action="{{ route('update.status', ['studentId' => $StudentMail->id_student]) }}" method="POST">
                    @csrf
