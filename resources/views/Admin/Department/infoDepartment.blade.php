      ...
      <tbody>
        ...
        {{-- {{dd($teacher)}} --}}
          <tr>
            ...
            <td>
              <form class="form_del" action="{{ route('deleteDepartment', $Department->id) }}" method="POST">
                @csrf
                @method('DELETE')

                <button type="submit" class="btn btn-danger">Xóa</button>
                <button class="btn btn-xs btn-primary" style="margin-left:10px;padding:0;"><a href="{{route('editDepartment',$Department->id)}}" style="color: #fff;
                  padding: 20px 20px;
                  font-size: 0.75rem;
                  line-height: 36px;
              }">
